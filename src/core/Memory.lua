...
function Memory.update()
	...
	Memory.current.timestop_enabled = memory.readbyte(Addresses[Settings.address_source_index].timestop_enabled)
	Memory.current.play_mode = memory.readwordsigned(Addresses[Settings.address_source_index].play_mode)
	...
	Memory.current.mario_h_speed = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_h_speed))
	Memory.current.mario_v_speed = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_v_speed))
	Memory.current.mario_x_sliding_speed = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_x_sliding_speed))
	Memory.current.mario_z_sliding_speed = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_z_sliding_speed))
	Memory.current.mario_x = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_x))
	Memory.current.mario_y = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_y))
	Memory.current.mario_z = MoreMaths.dec_to_float(memory.readdword(Addresses[Settings.address_source_index].mario_z))
	Memory.current.mario_pitch = memory.readwordsigned(Addresses[Settings.address_source_index].mario_pitch)
	Memory.current.mario_yaw_vel = memory.readwordsigned(Addresses[Settings.address_source_index].mario_yaw_vel)
	Memory.current.mario_pitch_vel = memory.readwordsigned(Addresses[Settings.address_source_index].mario_yaw_vel)
